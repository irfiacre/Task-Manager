...
export default ({ route, navigation }) => {
  ...
  const signInWithGoogleAsync = async () => {
    try {
      const result = await Google.logInAsync({
        behavior: "web",
        androidClientId:
          "618971057985-i03aasm7lcitpkocgjtn9ojhmig0vk2f.apps.googleusercontent.com",
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.wrapper}>
        <Text style={styles.text}>or Login with an Email</Text>
      </View>

      <View style={styles.wrapper}>
        <AuthFormComponent onSubmit={onUserLoggedIn} />
      </View>
    </View>
  );
};
